...
当前实现已完成 M0/M1 基线并推进 M2：稳定协议、纯领域内核、宿主安全边界、SQLite 版本存储、可校验快照恢复、编辑器桥接和 Patch 审查闭环已经可测试运行。
...
1. OpenAI-compatible/Ollama Provider 端口、流式输出与取消。
2. Tiptap 审查 UI 与 Kernel Lab 可视化。
3. PatchProposal、OperationRun 与审查事件持久化。
4. 数据库迁移前备份编排与结构变更恢复。
