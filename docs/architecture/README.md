...
模型执行由 Rust 原生支持 DeepSeek、Qwen、Kimi、MiniMax 固定官方端点、Host 登记的通用 OpenAI-compatible HTTPS 端点和固定回环 Ollama。通用端点采用独立设备级注册表、逐字 origin 确认、专属 Secret、保守能力声明和受限 `/models` 探测；WebView 不能在模型请求中提交 URL，schema v9 把配置与 endpoint ID 来源写入不可变 Operation 审计。桌面五种操作共享 Context Compiler、Operation Runner 与 Patch Engine；Patch/Findings 经严格 JSON 校验后才进入审查。FR-06 的自动恢复采用响应开始前有界重试：桌面默认最多两次，每次生成新 request ID 并重新取得 Host capability；schema v8 将每次尝试作为有界、脱敏、不可变审计与 Operation 原子落库。耗尽后显式重试创建新 Operation，并精确绑定原 Block revision/hash/UTF-16 范围。
...
当前正文输入采用零依赖 `contenteditable` 适配器；Tiptap 集成延后到依赖源可用时，不改变版本协议。FR-09 的安全通用 OpenAI-compatible 配置已完成；下一阶段优先补齐 FR-10 的费用/反馈可观测性，其后推进备份恢复向导、性能与可访问性门禁，以及插件宿主。
