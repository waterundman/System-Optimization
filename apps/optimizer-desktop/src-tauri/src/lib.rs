...
use optimizer_host::{
    ...
    CancelModelRequestResponse, ChangeDocumentDepthSpec, CheckpointSummary, CompareDocumentsSpec,
    ConfirmedContextPacket, CreateDocumentResponse, CreateDocumentSpec, CreateKnowledgeItemSpec,
    CreateReviewCandidateBranchResponse, CreateReviewCandidateBranchSpec, CreateStyleSampleSpec,
    DocumentDepthDirection, DocumentDiffResult, DocumentMoveDirection, DocumentMutationResponse,
    ExportDiagnosticsResponse, ExportJsonResponse, ExportMarkdownResponse,
    ExportProjectBackupResponse, ImportProjectBackupResponse, KnowledgeContextCandidate,
    KnowledgeContextSpec, KnowledgeItem, ModelAuthorizationScope, ModelExecutionHost,
    ModelExecutionRequest, ModelExecutionSummary, ModelGatewayError, ModelProviderId,
    ModelRequestAuthorization, ModelStreamEvent, NewProjectSpec, OllamaModelList,
    OpenAICompatibleModelList, OpenedProject, OperationAuditResponse, OperationCommandError,
    OperationContextCandidate, OperationContextSpec, OperationInsightsResponse,
    ...
    SummaryInvalidation, SummaryRefreshReport, TimelineEventsResponse, TrustedModelEndpoint,
    TrustedModelEndpointError, TrustedModelEndpointRegistry, UserConfigError, UserConfigRegistry,
...
pub struct DesktopState {
    ...
    user_config: Arc<Mutex<UserConfigRegistry>>,
...
impl DesktopState {
    pub fn new(secrets: Arc<dyn SecretStore>) -> Self {
        ...
        Self {
            ...
            trusted_endpoints,
            user_config: Arc::new(Mutex::new(UserConfigRegistry::memory())),
            secrets,
            models,
        }
    }
    ...
    pub fn configure_user_locale(&self, path: impl AsRef<Path>) -> Result<(), UserConfigError> {
        let registry = UserConfigRegistry::open(path)?;
        *self
            .user_config
            .lock()
            .map_err(|_| {
                UserConfigError::InvalidConfig("user config registry lock is poisoned")
            })? = registry;
        Ok(())
    }
    ...
    pub fn set_user_locale(
        &self,
        input: SetUserLocaleRequest,
    ) -> CommandResult<UserLocaleResponse> {
        input.validate()?;
        let locale = self
            .user_config_registry()?
            .set_locale(&input.locale)
            .map_err(CommandError::from)?;
        Ok(UserLocaleResponse {
            schema_version: 1,
            locale,
        })
    }

    pub fn get_user_locale(&self) -> CommandResult<UserLocaleResponse> {
        let locale = self.user_config_registry()?.locale().to_owned();
        Ok(UserLocaleResponse {
            schema_version: 1,
            locale,
        })
    ...
    pub fn export_json(&self) -> CommandResult<ExportJsonResponse> {
        self.current_project()?
            .export_json()
            .map_err(CommandError::from)
    }

    pub fn export_diagnostics(&self) -> CommandResult<ExportDiagnosticsResponse> {
        self.current_project()?
            .export_diagnostics()
            .map_err(CommandError::from)
    }
    ...
    pub fn list_timeline_events(&self) -> CommandResult<TimelineEventsResponse> {
        self.current_project()?
            .timeline_events()
            .map_err(CommandError::from)
    }
    ...
    ) -> CommandResult<DocumentDiffResult> {
        input.validate()?;
        self.current_project()?
            .compare_documents(&CompareDocumentsSpec {
                snapshot_id_a: input.snapshot_id_a,
                snapshot_id_b: input.snapshot_id_b,
                document_id_a: input.document_id_a,
                document_id_b: input.document_id_b,
            })
            .map_err(CommandError::from)
    }
    ...
    pub fn export_project_backup(
        &self,
        input: ExportProjectBackupRequest,
    ) -> CommandResult<ExportProjectBackupResponse> {
        input.validate()?;
        let project = self.current_project()?;
        let info = project.info().map_err(CommandError::from)?;
        project
            .export_project_backup(&optimizer_host::ExportProjectBackupRequest {
                project_id: info.project_id,
                include_endpoints: input.include_endpoints,
                include_recent: input.include_recent,
    ...
    pub fn import_project_backup(
        &self,
        input: ImportProjectBackupRequest,
    ) -> CommandResult<ImportProjectBackupResponse> {
        input.validate()?;
        OpenedProject::import_project_backup(&optimizer_host::ImportProjectBackupRequest {
    ...
    pub fn get_operation_insights(&self) -> CommandResult<OperationInsightsResponse> {
        let project = self.current_project()?;
        let project_id = project.info().map_err(CommandError::from)?.project_id;
        project
            .operations()
            .get_operation_insights(&project_id)
            .map_err(CommandError::from)
    }
    ...
    fn user_config_registry(&self) -> CommandResult<MutexGuard<'_, UserConfigRegistry>> {
        self.user_config
            .lock()
            .map_err(|_| CommandError::state_unavailable())
    }
...
impl From<UserConfigError> for CommandError {
...
#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct SetUserLocaleRequest {
    pub schema_version: u32,
    pub locale: String,
}

impl SetUserLocaleRequest {
    fn validate(&self) -> CommandResult<()> {
        if self.schema_version != 1 {
            return Err(CommandError::unsupported_request_schema(
                self.schema_version,
...
impl ExportProjectBackupRequest {
    fn validate(&self) -> CommandResult<()> {
        if self.schema_version != 1 {
            return Err(CommandError::unsupported_request_schema(
                self.schema_version,
...
impl ImportProjectBackupRequest {
    fn validate(&self) -> CommandResult<()> {
        if self.schema_version != 1 {
            return Err(CommandError::unsupported_request_schema(
                self.schema_version,
        ...
        if self.archive_path.trim().is_empty() || self.target_directory.trim().is_empty() {
            return Err(CommandError::basic(
                "PROJECT_BACKUP_PATH_INVALID",
                "archivePath and targetDirectory must be non-empty strings",
            ));
        }
        Ok(())
    }
...
async fn set_user_locale(
    ...
    spawn_host_task(state, move |state| state.set_user_locale(input)).await
}

#[tauri::command]
async fn get_user_locale(state: State<'_, DesktopState>) -> CommandResult<UserLocaleResponse> {
    spawn_host_task(state, DesktopState::get_user_locale).await
}
...
#[tauri::command]
async fn export_json(state: State<'_, DesktopState>) -> CommandResult<ExportJsonResponse> {
    spawn_host_task(state, DesktopState::export_json).await
}

#[tauri::command]
async fn export_diagnostics(
    state: State<'_, DesktopState>,
) -> CommandResult<ExportDiagnosticsResponse> {
    spawn_host_task(state, DesktopState::export_diagnostics).await
}
...
async fn list_timeline_events(
    state: State<'_, DesktopState>,
) -> CommandResult<TimelineEventsResponse> {
    spawn_host_task(state, DesktopState::list_timeline_events).await
}
...
async fn compare_documents(
    ...
    spawn_host_task(state, move |state| state.compare_documents(input)).await
}
...
async fn export_project_backup(
    ...
    spawn_host_task(state, move |state| state.export_project_backup(input)).await
}

#[tauri::command]
async fn import_project_backup(
    input: ImportProjectBackupRequest,
    state: State<'_, DesktopState>,
) -> CommandResult<ImportProjectBackupResponse> {
    spawn_host_task(state, move |state| state.import_project_backup(input)).await
}
...
#[tauri::command]
async fn get_operation_insights(
    state: State<'_, DesktopState>,
) -> CommandResult<OperationInsightsResponse> {
    spawn_host_task(state, DesktopState::get_operation_insights).await
}
    ...
    fn tauri_security_configuration_is_local_and_covers_every_registered_command() {
        ...
        assert_eq!(
            ...
            json!([
                ...
                "allow-version-read",
                "allow-version-write",
                "allow-project-backup",
                "allow-operation-write",
                "allow-operation-audit-read",
                "allow-operation-insights-read",
